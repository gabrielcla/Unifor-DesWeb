...
router.post('/'+tabela, async function(req, res, next){
    try{
        ...
        const id_quarto = customer['id_quarto'];
        const db = await connect();

        // Se for passado id_usuario e id_quarto válidos como parâmetro na requisição, adiciona o elemento a tabela
        if (id_usuario && id_quarto) {
            try {
                if (await db.collection("usuario").findOne({_id: new ObjectId(id_usuario)})) {

                    try {
                        if (await db.collection("quarto").findOne({_id: new ObjectId(id_quarto)})) {
                            // Cria nova reserva
                            res.json(await db.collection(tabela).insertOne(customer));
                        } else {
                            res.send("ERRO: Não existe quarto com o id_quarto fornecido!");
                        }
                    } catch {
                        res.send("ERRO: id_quarto Inválido!");
                    }
            ...
            res.send("ERRO: Forneça id_usuario e id_quarto!");    
        }
    }
    catch(ex){
      console.log(ex);
